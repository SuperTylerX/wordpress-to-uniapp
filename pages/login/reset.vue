			...
			<form @submit="reset">
				<view class="input-wrap">
					...
					<view class="password input-item">
						...
						<input class="input" type="password" confirm-type="next" placeholder="请输入新密码" name="password"
							v-model="form.password" maxlength="20" />
					</view>
					<view class="password input-item">
						<u-icon class="icon" size="22" name="lock"></u-icon>
						<input class="input" type="password" confirm-type="next" placeholder="请重复新密码" name="repassword"
					...
					<view class="emailCaptcha input-item">
						...
						<input class="input" type="text" confirm-type="done" placeholder="请输入邮箱验证码" @confirm="reset"
...
<script>
	import { getGraphicCaptcha, getEmailCaptcha, userRegister, userResetPassword } from "../../utils/http.js";
		...
		methods: {
			...
			async reset() {
				...
				try {
					const {
						data: { code, message }
					} = await userResetPassword(reqParamsObj);
